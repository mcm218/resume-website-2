...
export function FilterToolbar() {
  ...
  const expanded = collapsedByHand ?? !narrow;
  ...
  return (
    <div
      className={`fixed top-1/2 z-10 flex -translate-y-1/2 transition-[left] duration-250 ease-in-out ${
        expanded ? 'left-4' : '-left-[70px]'
      }`}
    >
      <ul
        aria-label="Filter experience by skill"
        className="flex max-h-[50vh] flex-col items-center gap-2.5 overflow-y-scroll rounded-[10px] bg-black/70 px-2.5 py-5 [scrollbar-width:none] max-sm:bg-black"
      >
        {SKILL_IDS.map((id) => (
          <li key={id}>
            <button
      ...
      <button
        ...
        onClick={() => setCollapsedByHand(!expanded)}
